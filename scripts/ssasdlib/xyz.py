...
def create_options(ext_name: str, script: type, AxisOptionClass: type, axis_options: list):
    with AxisOptions(AxisOptionClass, axis_options) as opts:
        ...
        options = [
            ...
            define('Max downsample', 1, int, choices=[1, 2, 4, 8]),
            define('Scale', 2, int, choices=[2, 4, 8, 16]),
            define('End step ratio', 3, float),
            define('Interpolation', 4, str, choices=['slice', 'nearest', 'nearest_exact', 'bilinear', 'bicubic']),
        ]
